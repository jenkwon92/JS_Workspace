/*총알을 정의한다*/
class Bullet{
  //총알이 태어날 때 ㅜㅂ여할 속성들을 생성자에서 처리한다
  ...
  tick(){
    ...
    //총알이 한걸음 한걸음 진행할 때마다, 배열에 존재하는 모든 적구늘 대상으로 충동했는지 여부를 판단할 것임
      for(var i=0; i<enemyArray.length; i++){
        var result=collisionCheck(this.img, enemyArray[i].img);
        if(result){
          //alert("맞췄어!");
          //너죽고(적군배열에서 요소삭제splice(index,갯수),화면에서도 삭제 removechild) 
          var img = enemyArray[i].img;
          wrapper.removeChild(img);
          enemyArray.splice(i,1);
  
          //나죽자(총알 배열에서 요소삭제splice(index,갯수), 화면에서도 삭제 removeChild) 
          wrapper.removeChild(this.img); //화면에서 제거
          var index = bulletArray.indexOf(this); //현재 총알의 인스턴스가 배열의 몇번때에 존재하는지 알아냄
